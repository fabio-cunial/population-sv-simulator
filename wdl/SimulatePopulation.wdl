...
task DeleteBucketDir {
    ...
    command <<<
        gsutil rm -rf ~{bucket_address} || true
    >>>
    output {
    }
    runtime {
    ...
    command <<<
        ...
        for ID1 in $(seq ~{id_from} 2 ${ID_TO}); do
            ...
            for LENGTH in ${LENGTHS}; do
                ...
                bash ~{work_dir}/haplotype2reads.sh ${ID1} ${ID2} ~{length_min} ~{length_max} ${LENGTH} ~{length_stdev} ~{max_coverage} ~{bucket_address} &> ${LOG_FILE}
                ...
                bash ~{work_dir}/reads2svs.sh ${READS_FILE} ${ID1} ${LENGTH} ~{min_coverage} ~{max_coverage} ${COVERAGES} ~{reference_fa} ~{reference_mmi} ~{reference_tandem_repeats} ${CHECKPOINT_FILE} ~{bucket_address} ~{use_pbsv} ~{use_sniffles1} ~{use_sniffles2} ~{use_hifiasm} 
                #&> ${LOG_FILE}
                #gsutil cp ${LOG_FILE} ~{bucket_address}/logs/
