function analyze_repeatFractions(MATRIX_DIR)
    ...
    for clr = [1:length(CALLERS)]
        for rf = [1:length(REPEAT_FRACTIONS)]
            ...
            for ms = [1:length(MEASURES)]
                ...
                title({MEASURES{ms},CALLERS{clr},sprintf('repeats>=%s',REPEAT_FRACTIONS{rf})}, 'fontsize', FONTSIZE);
            endfor
            for i = [1:3]
    ...
    for clr = [1:length(CALLERS)]
        for rf = [1:length(REPEAT_FRACTIONS)]
            ...
            for ms = [1:length(MEASURES)]
                ...
                xlabel('avg read length'); axis square; grid on; set(gca, 'fontsize', FONTSIZE);
                title({MEASURES{ms},CALLERS{clr},sprintf('repeats>=%s MERGE AND JOINT',REPEAT_FRACTIONS{rf})}, 'fontsize', FONTSIZE);
            endfor
            for i = [1:3]
                subplot(2,length(MEASURES)/2,i); xlim([READ_LENGTHS(1)-DELTA, READ_LENGTHS(length(READ_LENGTHS))+DELTA]);
            endfor
